...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not the_place.is_hive :
            if self.upper_bound!=-1 and times_found>self.upper_bound:
                break
            if the_place.bees :
                if times_found>=self.lower_bound:
                    found_place=the_place
                    break

            the_place=the_place.entrance
            times_found+=1
        if found_place is None  or found_place.is_hive  :
            return None
        return random_bee(found_place.bees) # REPLACE THIS LINE
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health<=amount:
            bees_amount+=self.damage
        for i in copy_bees:
            i.reduce_health(bees_amount)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.cooldown>0:
            ...
            kill_bee=random_bee(self.place.bees) # REPLACE THIS LINE
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        return self.ant_contained is  None and isinstance(other ,Ant)and other.is_container==False
        # END Problem 8a
...
class ScubaThrower(ThrowerAnt):
    ...
    name = 'Scuba'
    is_waterproof = True
    implemented = True#不加也行，不过为了美观还是加上
    def __init__(self, health=1):
        super().__init__(health)
# END Problem 11
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while the_place:
            if the_place.ant:
                if the_place.ant.is_container and the_place.ant.ant_contained:
                    the_place.ant.ant_contained.double()
                the_place.ant.double()
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        the_place=self.place
        while not the_place.is_hive:
            for bee_i in the_place.bees:
                result[bee_i]=distance
            if the_place.ant and the_place.ant!=self:
                result[the_place.ant]=distance
            the_place=the_place.entrance
            distance+=1
        return result
        # END Problem EC 4
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem EC 4
        result= self.damage-1/4*distance-1/16*self.insects_shot
