...
def interleaved_sum(n, odd_func, even_func):
    ...
    """
    "*** YOUR CODE HERE ***"
    def sum_inner(odd_start):
        if odd_start<n:
            return odd_func(odd_start)+even_func(odd_start+1)+sum_inner(odd_start+2)
        elif odd_start==n:
            return odd_func(odd_start)
        return  0
    return  sum_inner(1)
...
def count_dollars_upward(total):
    ...
    #     else :
    #         return  count_bigger(m-next_larger_dollar(n),n)+count_bigger(m,next_larger_dollar(n))
...
def make_anonymous_factorial():
    ...
    True
    """
    return (lambda n:(lambda f:f(f,n))(lambda f,M:1 if M<2 else mul(M,f(f,M-1))))
